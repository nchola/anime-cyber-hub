          ...
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {current.genres.slice(0, 3).map((genre) => (
                <span
                  key={genre.mal_id}
                  className="text-xs font-medium py-1 px-3 rounded-full bg-cyber-purple/30 text-cyber-accent border border-cyber-accent/30 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:bg-cyber-purple/50"
                >
                  {genre.name}
                </span>
              ))}
              ...
              {current.favorites && current.favorites > 0 && (
                <span className="text-xs font-medium py-1 px-3 rounded-full bg-pink-500/20 text-pink-300 border border-pink-500/30 backdrop-blur-sm flex items-center gap-1">
                  ...
                  {current.favorites.toLocaleString()}
                </span>
              )}
            </div>
